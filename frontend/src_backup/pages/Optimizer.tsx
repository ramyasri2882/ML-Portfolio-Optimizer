...
export default function Optimizer() {
  ...
  const [resultsPortfolio, setResultsPortfolio] = useState<any[]>([]);

  const handleGenerate = () => {
    setIsGenerating(true);
    setShowResults(false);
    
    fetch('http://localhost:8000/api/portfolio-optimization')
      .then(res => res.json())
      .then(data => {
        const filtered = data.filter((a: any) => selectedStocks.includes(a.symbol));
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-slate-900/40 backdrop-blur-xl rounded-2xl border border-indigo-500/20 shadow-[0_0_15px_rgba(0,0,0,0.5)] overflow-hidden hover:border-indigo-500/30 transition-colors flex flex-col lg:flex-row">
        
        {/* Configuration Panel */}
        <div className="w-full lg:w-[450px] flex-shrink-0 bg-slate-900/60 backdrop-blur-sm border-r border-indigo-900/50 p-6 space-y-8">
          <div className="flex items-center gap-2 mb-2 text-primary-700 font-semibold border-b border-indigo-900/40 pb-4">
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-8 pr-4 py-3 bg-slate-900/40 backdrop-blur-md border border-indigo-900/40 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-shadow font-medium"
                ...
                <label 
                  ...
                  className={cn(
                    ...
                    selectedStocks.includes(stock.symbol) 
                      ? "bg-indigo-950/50 border-indigo-500/50 text-indigo-300 shadow-[0_0_10px_rgba(99,102,241,0.2)]" 
                      : "bg-slate-900/40 backdrop-blur-md border-indigo-900/40 text-slate-300 hover:bg-slate-900/40"
                  )}
                >
                  <input 
                    type="checkbox" 
          ...
          <div className="space-y-3">
            ...
            <select 
              ...
              className="w-full p-3 bg-slate-900/40 backdrop-blur-md border border-indigo-900/40 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm font-medium text-slate-200"
            >
          ...
          <div className="pt-4 border-t border-indigo-900/40">
            <button 
              ...
              className="w-full py-4 bg-slate-900 border border-indigo-500/50 text-indigo-400 hover:text-indigo-300 hover:bg-indigo-950/50 font-bold rounded-xl hover:shadow-[0_0_20px_rgba(99,102,241,0.5)] hover:border-indigo-400 hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2 shadow-lg shadow-primary-500/30 disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:translate-y-0"
            >
              {isGenerating ? (
                <>
        ...
        {/* Status / Initial View */}
        <div className="flex-1 bg-slate-900/40 backdrop-blur-md flex items-center justify-center p-8 min-h-[500px]">
          {!isGenerating && !showResults && (
            <div className="text-center max-w-md">
              ...
              <p className="text-slate-400">Configure your parameters on the left and click generate to build an efficient frontier based portfolio using machine learning predictions.</p>
            </div>
          )}
      ...
      {/* Results Section */}
      {showResults && (
        <div id="results-section" className="space-y-6 pt-6 border-t border-indigo-900/40 animate-in slide-in-from-bottom-8 duration-700 fade-in">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <h2 className="text-2xl font-bold text-white">Recommended Portfolio</h2>
            <div className="flex gap-2">
              <button className="px-4 py-2 bg-slate-900/40 backdrop-blur-md border border-indigo-900/40 text-slate-200 font-medium rounded-lg hover:bg-slate-900/40 transition-colors flex items-center gap-2 text-sm shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-slate-900/40 backdrop-blur-md p-5 rounded-xl border border-emerald-200 shadow-sm shadow-emerald-100">
            ...
            <div className="bg-slate-900/40 backdrop-blur-md p-5 rounded-xl border border-rose-200 shadow-sm shadow-rose-100">
            ...
            <div className="bg-slate-900/40 backdrop-blur-md p-5 rounded-xl border border-primary-200 shadow-sm shadow-primary-100">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Chart */}
            <div className="bg-slate-900/40 backdrop-blur-md p-6 rounded-xl border border-indigo-900/40 shadow-sm flex flex-col items-center">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <RechartsPieChart>
                    ...
                    <RechartsTooltip 
                      ...
                      contentStyle={{ borderRadius: '8px', border: '1px solid #1e293b', backgroundColor: '#0f172a', color: '#f8fafc', fontWeight: 'bold' }}
                    />
            ...
            {/* Table */}
            <div className="bg-slate-900/40 backdrop-blur-md rounded-xl border border-indigo-900/40 shadow-sm overflow-hidden flex flex-col">
                ...
                <table className="w-full text-sm text-left">
                  ...
                  <tbody>
                    {resultsPortfolio.map((asset, i) => (
                      <tr key={asset.symbol} className={cn("border-b border-indigo-900/30 hover:bg-slate-900/40", i === resultsPortfolio.length - 1 ? 'border-b-0' : '')}>
                        <td className="px-5 py-3 flex items-center gap-3">
                          <div className="w-3 h-3 rounded-full" style={{ backgroundColor: asset.color }}></div>
                          <span className="font-bold text-white">{asset.symbol}</span>
                        </td>
                        <td className="px-5 py-3 text-right font-semibold text-white">{asset.allocation}%</td>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4">
            ...
            <button 
              ...
              className="flex-1 py-4 bg-slate-900/40 backdrop-blur-md text-slate-200 border-2 border-indigo-900/40 font-bold rounded-xl hover:bg-slate-900/40 hover:border-slate-300 transition-colors flex items-center justify-center gap-2"
            >
