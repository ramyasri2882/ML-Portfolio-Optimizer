...
export default function MLPrediction() {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/ml-predictions')
      .then(res => res.json())
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
        ...
        <button 
          ...
          className="px-5 py-2.5 bg-slate-900 border border-indigo-500/50 text-indigo-400 hover:text-indigo-300 hover:bg-indigo-950/50 font-semibold rounded-xl hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2 shadow-md shadow-primary-500/20"
        >
      ...
      <div className="bg-slate-900/40 backdrop-blur-xl p-6 rounded-2xl border border-indigo-500/20 shadow-[0_0_15px_rgba(0,0,0,0.5)] overflow-x-auto">
        <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-6">Prediction Pipeline</h3>
        <div className="flex items-center min-w-max">
          <div className="flex flex-col items-center">
            <div className="w-16 h-16 bg-slate-800/40 rounded-xl flex items-center justify-center border border-indigo-900/40 z-10 relative">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-xs font-semibold text-slate-200 mt-3">Predicted Return</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Table */}
        <div className="lg:col-span-2 bg-slate-900/40 backdrop-blur-xl rounded-2xl border border-indigo-500/20 shadow-[0_0_15px_rgba(0,0,0,0.5)] overflow-hidden hover:border-indigo-500/30 transition-colors flex flex-col">
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {predictions.map((pred, i) => (
                  <tr key={pred.symbol} className={cn("border-b border-indigo-900/30 hover:bg-slate-900/40 transition-colors", i === predictions.length - 1 ? 'border-b-0' : '')}>
                    ...
                    <td className="px-6 py-4">
                      <span className={cn(
                        ...
                        pred.signal === 'BUY' ? "bg-emerald-950/40 border border-emerald-500/30 text-emerald-700" :
                        pred.signal === 'SELL' ? "bg-rose-950/40 border border-rose-500/30 text-rose-700" :
        ...
        <div className="space-y-6 flex flex-col">
          <div className="bg-slate-900/40 backdrop-blur-md p-5 rounded-xl border border-indigo-900/40 shadow-sm">
          ...
          <div className="bg-slate-900/40 backdrop-blur-md p-5 rounded-xl border border-indigo-900/40 shadow-sm flex-1 flex flex-col min-h-[300px]">
