  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-64 flex-shrink-0 glass-panel rounded-xl overflow-hidden flex flex-col h-[600px]">
          <div className="p-4 border-b border-cyan-900/40 bg-[#030B1C]/60">
              ...
              <Link
                ...
                className={cn(
                  ...
                  stock.symbol === selectedStock.symbol 
                    ...
                    : "hover:bg-[#030B1C]/60 border border-transparent"
                )}
              >
                <div>
                  <div className={cn("font-bold text-sm", stock.symbol === selectedStock.symbol ? "text-cyan-400" : "text-slate-200")}>{stock.symbol}</div>
        ...
        <div className="flex-1 space-y-6">
          <div className="glass-card p-6 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <Link 
              to="/optimizer"
              className="px-6 py-2.5 bg-[#020817] border border-cyan-500/50 text-cyan-400 hover:text-cyan-300 hover:bg-cyan-950/40 font-semibold rounded-xl hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2 shadow-md shadow-primary-500/20"
            >
              Use This Stock
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {statCards.map((stat, i) => (
              <div key={i} className="bg-[#030B1C]/60 backdrop-blur-xl p-4 rounded-2xl border border-cyan-900/40 shadow-[0_4px_20px_rgba(0,0,0,0.6)] hover:-translate-y-1 transition-transform duration-300">
          ...
          {/* Tabs */}
          <div className="bg-[#030B1C]/60 backdrop-blur-xl border border-cyan-900/40 rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.6)] overflow-hidden hover:border-indigo-500/30 transition-colors flex flex-col h-[450px]">
            <div className="flex border-b border-cyan-900/40 bg-[#030B1C]/60 overflow-x-auto">
              {tabs.map(tab => (
                <button
                  key={tab.id}
                  ...
                  className={cn(
                    ...
                    activeTab === tab.id
                      ? "border-primary-600 text-cyan-400 bg-[#030B1C]/60 backdrop-blur-md"
            ...
            <div className="flex-1 p-6">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={chartData}>
                        ...
                        <RechartsTooltip 
                          ...
                          formatter={((value: any) => [`₹${value.toFixed(2)}`, 'Price']) as any}
                          labelStyle={{ color: '#64748b', marginBottom: '4px' }}
                        />
                        <Area type="monotone" dataKey="price" stroke="#00f0ff" filter="drop-shadow(0px 0px 4px rgba(0,240,255,0.8))" strokeWidth={2} fillOpacity={1} fill="url(#colorPrice)" />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={chartData}>
                        ...
                        <RechartsTooltip 
                          ...
                          formatter={((value: any) => [`${value}%`, 'Return']) as any}
                        />
                        <Line type="linear" dataKey="return_val" stroke="#8a2be2" filter="drop-shadow(0px 0px 4px rgba(138,43,226,0.8))" strokeWidth={2} dot={false} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                  ...
                  <div className="flex-1 min-h-0">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={chartData}>
                        ...
                        <RechartsTooltip 
                          contentStyle={{ borderRadius: '8px', border: '1px solid #1e293b', backgroundColor: '#0f172a', color: '#f8fafc' }}
                          formatter={((value: any) => [`${value}%`, 'Volatility']) as any}
                        />
                        <Line type="monotone" dataKey="volatility" stroke="#ff003c" filter="drop-shadow(0px 0px 4px rgba(255,0,60,0.8))" strokeWidth={2} dot={false} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              {activeTab === 'correlation' && (
                <div className="h-full flex flex-col">
                      ...
                      {stocks.map(s => (
                        <label key={s.symbol} className={cn(
                          ...
                          selectedCorrelationStocks.includes(s.symbol) 
                            ? "border-primary-500 bg-cyan-950/40 text-cyan-400 font-medium" 
                            : "border-cyan-900/40 hover:bg-[#030B1C]/60 text-slate-300"
                        )}>
                          <input 
                            type="checkbox" 
                            className="hidden"
                  ...
                  <div className="flex-1 overflow-auto border border-cyan-900/40 rounded-xl rounded-b-none border-b-0 bg-[#030B1C]/60 backdrop-blur-md">
                    {selectedCorrelationStocks.length < 2 ? (
                      ...
                      <table className="w-full text-sm text-left">
                        <thead className="text-xs text-slate-400 uppercase bg-[#030B1C]/60 sticky top-0 z-10 shadow-sm">
                          <tr>
                            <th className="px-6 py-3 font-semibold bg-[#030B1C]/60">Asset</th>
                            {selectedCorrelationStocks.map(sym => (
                              <th key={sym} className="px-6 py-3 font-semibold text-center bg-[#030B1C]/60">{sym}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {selectedCorrelationStocks.map((rowSym) => (
                            <tr key={rowSym} className="border-b border-cyan-900/30 hover:bg-[#030B1C]/60 transition-colors">
                              <td className="px-6 py-3 font-bold text-white bg-[#030B1C]/60 backdrop-blur-md sticky left-0 shadow-[1px_0_0_0_#f1f5f9] z-10">{rowSym}</td>
